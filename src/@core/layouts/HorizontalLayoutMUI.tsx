...
import AppBarContent from "./components/horizontal/appBarContent";
...
const HorizontalLayout = (props: LayoutProps) => {
	// ** Props
	const { hidden, children, contentHeightFixed, horizontalLayoutProps } = props;

	// ** Vars

	const { skin, appBar, navHidden, appBarBlur, contentWidth } = useSelector(
		(state: any) => state.layoutMUI,
	);
						...
						>
							<Toolbar
								...
								sx={{
									...
									minHeight: (theme) =>
										`${
											(theme.mixins.toolbar.minHeight as number) - 4 - (skin === "bordered" ? 1 : 0)
										}px !important`,
								}}
							>
								<>
									<Navigation
										{...props}
										horizontalNavItems={horizontalLayoutProps}
									/>
								</>
							</Toolbar>
						</Box>
